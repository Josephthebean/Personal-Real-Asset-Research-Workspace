...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            input_dir = Path(tmp) / "new"; input_dir.mkdir()
            (input_dir / "sample.md").write_text(SAMPLE, encoding="utf-8")
            result = subprocess.run([sys.executable, "scripts/ingest_reports.py", "--dry-run", "--input-dir", str(input_dir)], cwd=ROOT, text=True, capture_output=True)
        ...
        if missing:
            print(f"Missing detected sections: {sorted(missing)}"); return 1
        print("Report ingestion check passed."); return 0
    finally:
